...
- `tools/contest_utils.py` — `contest_name` の数字省略形（例: `426` → `abc426`）を解決する共有ヘルパー。`create_new_contest.py` / `download_test.py` / `run_test.py` から参照される
...
- `contest_name` は `create_new_contest.py` と同様、`dt`/`rt` どちらも数字のみ（例: `426`）を渡すと `abc426` として扱われる
